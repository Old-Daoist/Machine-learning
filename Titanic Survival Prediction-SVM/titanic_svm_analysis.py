...
bars   = ax.barh(cols_m, pcts_m, color=clrs_m, height=0.45,
...
ax.set_title('Figure 1 — Missing Values per Column')
ax.legend()
notes = ['Imputed: median by Pclass', 'Dropped: too many missing', 'Imputed: mode (S)']
for i, note in enumerate(notes):
    ax.text(83, i, note, va='center', fontsize=9, color='#555555')
...
sns.heatmap(hmap, annot=True, fmt='.4f', cmap='Blues',
            ...
            ax=axes[0], cbar_kws={'shrink': 0.8, 'label': 'Mean AUC'})
axes[0].set_title('Figure 2a — CV ROC-AUC Heatmap (C x Gamma)')
...
axes[1].set_xlim(0.82, 0.875)
axes[1].set_xlabel('Mean CV ROC-AUC')
axes[1].set_title('Figure 2b — Grid Search Rankings')
...
lin_prec = precision_score(y_test, svm_lin.predict(X_test_s))
lin_rec  = recall_score(y_test, svm_lin.predict(X_test_s))
lin_f1   = f1_score(y_test, svm_lin.predict(X_test_s))
lin_mcc  = matthews_corrcoef(y_test, svm_lin.predict(X_test_s))
...
b1 = ax.bar(x_pos - w/2, lin_vals, w, label='Linear SVM (C=1)',
            color=BLUE, alpha=0.85, edgecolor='white', linewidth=1.2)
b2 = ax.bar(x_pos + w/2, rbf_vals, w,
            label=f'RBF SVM (C={best_params["C"]}, gamma={best_params["gamma"]})',
            color=PURPLE, alpha=0.85, edgecolor='white', linewidth=1.2)
for bar in b1:
...
ax.set_ylim(0, 1.12)
ax.set_ylabel('Score')
ax.set_title('Figure 3 — Linear vs RBF SVM: All Metrics')
ax.axhline(0.8, color=GRAY, linestyle=':', linewidth=1, alpha=0.7, label='0.8 reference')
ax.legend(loc='upper right')
delta_lbl = (f"AUC delta = {improvement:+.4f}  —  "
             f"{'Meaningful improvement (>0.02)' if improvement > 0.02 else 'Not meaningful (delta <= 0.02)'}")
ax.text(0.5, 0.02, delta_lbl, transform=ax.transAxes, ha='center', fontsize=10,
        color='#333333',
...
ax.set_title('Figure 4 — ROC Curves: Linear vs RBF SVM')
ax.legend(loc='lower right', framealpha=0.9)
ax.set_xlim([0, 1])
ax.set_ylim([0, 1.02])
ax.annotate(f'delta AUC = {improvement:+.4f}', xy=(0.55, 0.52),
            fontsize=12, color=PURPLE, fontweight='bold',
            bbox=dict(boxstyle='round,pad=0.4', facecolor='#EDE7F6', edgecolor=PURPLE))
plt.tight_layout(pad=2)
...
axes[1].set_title('Figure 5b — Confusion Matrix Breakdown')
...
info_txt = (f"False Positive Rate : {fpr_val*100:.1f}%\n"
            ...
            f"(survivors correctly identified)")
axes[1].text(0.97, 0.97, info_txt, transform=axes[1].transAxes,
             va='top', ha='right', fontsize=10,
             bbox=dict(boxstyle='round,pad=0.6',
                       facecolor='#F3E5F5', edgecolor=PURPLE))
plt.tight_layout(pad=2.5)
...
summary_rows = [
    ...
    ("  Recall",              f"{recall_val*100:.1f}%  (survivors correctly identified)"),
]
...
for label, value in summary_rows:
    ...
    ax.text(0.05, y_pos, label, transform=ax.transAxes,
            fontsize=11.5 if is_header else 11,
            color='#1A237E' if is_header else '#212121',
            fontweight='bold' if is_header else 'normal',
            va='top', fontfamily='monospace')
    if value:
        ax.text(0.55, y_pos, value, transform=ax.transAxes,
                fontsize=11, color='#37474F', va='top', fontfamily='monospace')
    y_pos -= 0.037

ax.set_title('Figure 6 — Final Summary', fontsize=14, fontweight='bold', pad=15)
ax.add_patch(plt.Rectangle((0.02, 0.01), 0.96, 0.97,
             transform=ax.transAxes, fill=False,
             edgecolor=PURPLE, linewidth=2, clip_on=False))
plt.tight_layout(pad=2)
